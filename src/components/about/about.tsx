  ...
  return (
    <div className="w-full flex flex-col items-center justify-center min-h-screen bg-[var(--base-green)] py-24 sm:py-32 px-6 lg:px-8">
      <div className="max-w-3xl text-center">
        <h2 className="text-4xl font-bold text-white sm:text-5xl">SNGF, C'est Quoi?</h2>
        <p className="mt-4 text-lg text-white">
          Vous trouverez ici ce que vous recherchez en matière de graine et d'arbre
          de n'importe quel espèce
        </p>
      </div>

      <div className="mt-16 grid gap-10 md:grid-cols-3">
        {posts.map((post, index) => (
          <div key={index} className="bg-white rounded-lg shadow-lg object-fill overflow-hidden">
            <img src={post.image} alt={post.title} className="w-full h-60 max-h-48 object-cover" />
            <div className="p-6">
